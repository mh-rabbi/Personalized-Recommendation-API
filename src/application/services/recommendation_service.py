...
class RecommendationService:
    ...
    def get_personalized_recommendations(self, user_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        user_history = self.event_repo.get_top_products_by_user(user_id, 1)
        ...
        if not has_history:
            # Case 1: 100% Segment
            segment_products = self.event_repo.get_top_products_by_segment(age_group, gender, limit)
            ...
            personal_products = self.event_repo.get_top_products_by_user(user_id, limit_personal)
            ...
            top_categories = self.event_repo.get_top_categories_by_user(user_id, 5) # Get top 5 categories
            if top_categories:
                category_ids = [c[0] for c in top_categories]
                category_products = self.event_repo.get_top_products_by_categories(category_ids, limit_category * 2) # Fetch more to filtering duplicates
            ...
            if remaining_limit > 0:
                global_products = self.event_repo.get_top_products_global(limit * 2)
                count = 0
